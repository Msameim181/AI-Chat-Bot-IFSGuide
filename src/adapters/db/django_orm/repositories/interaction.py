...
from src.entities.interaction import Interaction as InteractionEntity
from src.adapters.db.django_orm.core.models.interaction import Interaction as InteractionModel
from dataclasses import asdict

class InteractionDBRepository(IInteractionDBRepository):
    def __init__(self, logger: logger):
        super().__init__()
        self.logger = logger
        self.MODEL = InteractionModel
        self.MODEL_ENTITY = InteractionEntity
    ...
    async def validate(self, obj: InteractionEntity) -> Any:
        try:
            obj = asdict(obj)
    ...
    async def create(self, obj: InteractionEntity) -> Any:
    ...
    async def update(self, obj: InteractionModel) -> Any:
        if not isinstance(obj, InteractionModel):
            return None
        try:
            await obj.asave()
    ...
    async def update_filter(self, filters: dict, obj: InteractionEntity) -> Any:
        obj = await self.validate(obj)
        obj = await self.remove_nulls(obj)
        obj = await self.remove_ids(obj)
        try:
    ...
    async def get(self, filters: dict) -> InteractionModel:
        return await self.MODEL.objects.filter(**filters).afirst()

    async def get_by_id(self, obj_id: int) -> InteractionModel:
