...
from src.adapters.db.django_orm.core.models.user import User as UserModel
from dataclasses import asdict

class UserDBRepository(IUserDBRepository):
    def __init__(self, in_logger: logger):
        super().__init__()
        self.logger = in_logger
        self.MODEL = UserModel
        self.MODEL_ENTITY = UserEntity
    ...
    async def update(self, obj: UserModel) -> Any:
    ...
    async def update_filter(self, filters: dict, obj: UserEntity) -> Any:
        obj = await self.validate(obj)
        obj = await self.remove_nulls(obj)
        obj = await self.remove_ids(obj)
        try:
    ...
    async def get(self, filters: dict) -> UserModel:
        return await self.MODEL.objects.filter(**filters).afirst()

    async def get_by_id(self, obj_id: int) -> UserModel:
